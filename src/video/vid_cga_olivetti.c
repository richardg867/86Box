            ...
            if (ogc->cga.cgadispon) {
                ...
                if (ogc->cga.cgamode & CGA_MODE_FLAG_HIGHRES) {
                    ...
                    if ((ogc->cga.displine % 8) == 0)
                        cli_render_cga(ogc->cga.memaddr / ogc->cga.crtc[1], ogc->cga.crtc[9] & 0x1f,
                                ogc->cga.crtc[1], 1,
                                ogc->cga.charbuffer, 0, sizeof(ogc->cga.charbuffer) - 1, 1,
                                ogc->cga.cgamode & 0x08, ogc->cga.cgamode & 0x20,
                                cursoraddr - ogc->cga.memaddr, !(ogc->cga.crtc[0x0a] & 0x20) && ((ogc->cga.crtc[0x0b] & 0x1f) >= (ogc->cga.crtc[0x0a] & 0x1f)));
#endif
                ...
                /* 40-col */
                    ...
                    if ((ogc->cga.displine % 8) == 0)
                        cli_render_cga(ogc->cga.memaddr / ogc->cga.crtc[1], ogc->cga.crtc[9] & 0x1f,
                                ogc->cga.crtc[1], 1,
                                ogc->cga.vram, ogc->cga.memaddr << 1, 0x3fff, 1,
                                ogc->cga.cgamode & 0x08, ogc->cga.cgamode & 0x20,
                                cursoraddr, !(ogc->cga.crtc[0x0a] & 0x20) && ((ogc->cga.crtc[0x0b] & 0x1f) >= (ogc->cga.crtc[0x0a] & 0x1f)));
#endif
                    for (x = 0; x < ogc->cga.crtc[CGA_CRTC_HDISP]; x++) {
                        if (ogc->cga.cgamode & CGA_MODE_FLAG_VIDEO_ENABLE) {
