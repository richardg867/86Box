            ...
            if (nga->cga.cgadispon) {
                ...
                if ((nga->cga.cgamode & CGA_MODE_FLAG_HIGHRES) && !(nga->cga.cgamode & CGA_MODE_FLAG_GRAPHICS)) {
                    ...
                    if ((nga->cga.displine % 8) == 0)
                        cli_render_cga(nga->cga.memaddr / nga->cga.crtc[1], nga->cga.crtc[9] & 0x1f,
                                nga->cga.crtc[1], 1,
                                nga->cga.charbuffer, 0, sizeof(nga->cga.charbuffer) - 1, 1,
                                nga->cga.cgamode & 0x08, nga->cga.cgamode & 0x20,
                                cursoraddr - nga->cga.memaddr, !(nga->cga.crtc[0x0a] & 0x20) && ((nga->cga.crtc[0x0b] & 0x1f) >= (nga->cga.crtc[0x0a] & 0x1f)));
#endif
                ...
                /* 40-col */
                    ...
                    if ((nga->cga.displine % 8) == 0)
                        cli_render_cga(nga->cga.memaddr / nga->cga.crtc[1], nga->cga.crtc[9] & 0x1f,
                                nga->cga.crtc[1], 1,
                                nga->cga.vram, nga->cga.memaddr << 1, 0x3fff, 1,
                                nga->cga.cgamode & 0x08, nga->cga.cgamode & 0x20,
                                cursoraddr, !(nga->cga.crtc[0x0a] & 0x20) && ((nga->cga.crtc[0x0b] & 0x1f) >= (nga->cga.crtc[0x0a] & 0x1f)));
#endif
                    /* for each text column */
                    for (x = 0; x < nga->cga.crtc[CGA_CRTC_HDISP]; x++) {
                        if (nga->cga.cgamode & CGA_MODE_FLAG_VIDEO_ENABLE) {
