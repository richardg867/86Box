...
ess_dac2_update(void *priv)
{
    ...
    for (; ess->ess_dac2_pos < sound_get_legacy_pos(ess->ess_dac2_source); ess->ess_dac2_pos++) {
...
sb_init(UNUSED(const device_t *info))
{
    ...
    sb->cms.source = sound_add_handler(sb_get_cms_buffer_sb2, sb);
...
sb_pro_v2_init(UNUSED(const device_t *info))
{
    ...
    sb->opl_enabled = device_get_config_int("opl");
    if (sb->opl_enabled)
        fm_driver_get(FM_YMF262 | FM_OPL3TIMER, &sb->opl, music_add_handler(sb_get_music_buffer_sbpro, sb));
...
sb_16_init(UNUSED(const device_t *info))
{
    ...
    sb->opl_enabled = device_get_config_int("opl");
    if (sb->opl_enabled)
        fm_driver_get((int) (intptr_t) info->local | FM_OPL3TIMER, &sb->opl, music_add_handler(sb_get_music_buffer_sb16_awe32, sb));
...
sb_16_reply_mca_init(UNUSED(const device_t *info))
{
    ...
    sb->opl_enabled = 1;
    fm_driver_get(FM_YMF262 | FM_OPL3TIMER, &sb->opl, music_add_handler(sb_get_music_buffer_sb16_awe32, sb));
...
sb_16_pnp_init(UNUSED(const device_t *info))
{
    ...
    sb->opl_enabled = 1;
    fm_driver_get((int) (intptr_t) FM_CQM | FM_OPL3TIMER, &sb->opl, music_add_handler(sb_get_music_buffer_sb16_awe32, sb));
...
sb_vibra16_pnp_init(UNUSED(const device_t *info))
{
    ...
    sb->opl_enabled = 1;
    fm_driver_get(FM_CQM | FM_OPL3TIMER, &sb->opl, music_add_handler(sb_get_music_buffer_sb16_awe32, sb));
...
sb_awe32_pnp_init(const device_t *info)
{
    ...
    sb->opl_enabled = 1;
    fm_driver_get((cqm ? FM_CQM : FM_YMF262) | FM_OPL3TIMER, &sb->opl, music_add_handler(sb_get_music_buffer_sb16_awe32, sb));
...
ess_x688_init(UNUSED(const device_t *info))
{
    ...
    fm_driver_get((info->local ? FM_ESFM : FM_YMF262) | FM_OPL3TIMER, &ess->opl, music_add_handler(sb_get_music_buffer_ess, ess));
...
ess_1x88_onboard_init(const device_t *info)
{
    ...
    fm_driver_get(FM_ESFM | FM_OPL3TIMER, &ess->opl, music_add_handler(sb_get_music_buffer_ess, ess));
    ...
    if (ess->dsp.sb_subtype >= SB_SUBTYPE_ESS_ES1888) {
        ess->ess_dac2_source = sound_add_handler(sb_get_buffer_ess_dac2, ess);
