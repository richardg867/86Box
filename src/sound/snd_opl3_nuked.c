...
{
    if ((dev->flags & FLAG_OPL3TIMER) && start)
...
nuked_opl3_drv_init(const device_t *info)
{
    ...
    dev->flags       = FLAG_CYCLES | ((params->chip_id & FM_OPL3TIMER) ? FLAG_OPL3TIMER : 0);
    dev->source      = params->source;
    if (((params->chip_id & FM_TYPE_MASK) == FM_YMF262) || ((params->chip_id & FM_TYPE_MASK) == FM_YMF289B))
        dev->flags |= FLAG_OPL3;
    else
        dev->status = 0x06;
