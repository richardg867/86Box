...
cs423x_init(const device_t *info)
{
    ...
    fm_driver_get(FM_YMF262, &dev->sb->opl, music_add_handler(sb_get_music_buffer_sbpro, dev->sb)); /* Init the SBPro OPL3 since sb_pro_compat_init does not */

    /* Initialize RAM, registers and WSS codec. */
    cs423x_reset(dev);
    dev->ad1848.source = sound_add_handler(cs423x_get_buffer, dev);
