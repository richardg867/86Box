...
optimc_init(const device_t *info)
{
    ...
    fm_driver_get(optimc->fm_type | FM_OPL3TIMER, &optimc->sb->opl, (optimc->fm_type == FM_YMF278B) ? wavetable_add_handler(sb_get_music_buffer_sbpro, optimc->sb) : music_add_handler(sb_get_music_buffer_sbpro, optimc->sb));
    io_sethandler(optimc->cur_addr + 0, 0x0004, optimc->sb->opl.read, NULL, NULL, optimc->sb->opl.write, NULL, NULL, optimc->sb->opl.priv);
    io_sethandler(optimc->cur_addr + 8, 0x0002, optimc->sb->opl.read, NULL, NULL, optimc->sb->opl.write, NULL, NULL, optimc->sb->opl.priv);
    io_sethandler(0x0388, 0x0004, optimc->sb->opl.read, NULL, NULL, optimc->sb->opl.write, NULL, NULL, optimc->sb->opl.priv);
    ...
    if (optimc->type == OPTI_930) {
        ...
        optimc->sb->dsp.source = sound_add_handler(opti930_get_sbpro_buffer, optimc);
    } else {
        optimc->ad1848.source = sound_add_handler(optimc_get_buffer, optimc);
        optimc->sb->dsp.source = sound_add_handler(optimc_get_sbpro_buffer, optimc);
    }
    if (optimc->type == OPTI_930)
