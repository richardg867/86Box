    ...
    while (data->on) {
        ...
        if (data->format == SOUND_FLOAT32) {
            ...
            if (buf_pos >= data->buf_size) {
                sound_backend_buffer(data->source, data->out_buffer, data->buf_size);
                buf_pos = 0;
            }
        } else {
            ...
            buf_pos += seg_bytes;
            if (buf_pos >= data->buf_size) {
                sound_backend_buffer(data->source, data->out_buffer_int16, data->buf_size);
                buf_pos = 0;
            }
        }
    }
...
soundcanvas_init(UNUSED(const device_t *info))
{
    ...
    uint8_t channels = 2;
    sound_backend_set_format(data->source, &data->format, &channels, &data->sample_rate);
