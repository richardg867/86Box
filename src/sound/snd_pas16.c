...
pas16_init(const device_t *info)
{
    ...
    pas16->has_scsi = (!pas16->type) || (pas16->type == 0x0f);
    fm_driver_get(FM_YMF262 | FM_OPL3TIMER, &pas16->opl, music_add_handler(pas16->type ? pas16_get_music_buffer : pasplus_get_music_buffer, pas16));
    ...
    if (pas16->type) {
        pas16->source = sound_add_handler(pas16_get_buffer, pas16);
        pas16->dsp.source = sound_add_handler(pas16_get_sb_buffer, pas16);
        sound_set_cd_audio_filter(pas16_filter_cd_audio, pas16);
        if (device_get_config_int("control_pc_speaker"))
            sound_set_pc_speaker_filter(pas16_filter_pc_speaker, pas16);
    } else {
        pas16->source = sound_add_handler(pasplus_get_buffer, pas16);
        pas16->dsp.source = sound_add_handler(pasplus_get_sb_buffer, pas16);
        sound_set_cd_audio_filter(pasplus_filter_cd_audio, pas16);
        if (device_get_config_int("control_pc_speaker"))
            sound_set_pc_speaker_filter(pasplus_filter_pc_speaker, pas16);
    }
