...
azt_init(const device_t *info)
{
    ...
    azt2316a->ad1848.source = sound_add_handler(azt2316a_get_buffer, azt2316a);
    azt2316a->sb->dsp.source = sound_add_handler(azt2316a_get_sbpro_buffer, azt2316a);

    if ((azt2316a->type == SB_SUBTYPE_CLONE_AZT2316A_0X11) || (azt2316a->type == SB_SUBTYPE_CLONE_AZT2316R_0X12)) {
        if (azt2316a->sb->opl_enabled)
            fm_driver_get(FM_YMF262 | FM_OPL3TIMER, &azt2316a->sb->opl, music_add_handler(sb_get_music_buffer_sbpro, azt2316a->sb));
    }
    else {
        if (azt2316a->sb->opl_enabled) {
            azt2316a->sb->opl_mixer = azt2316a;
