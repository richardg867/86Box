...
uint32_t
rom_get_pe_resource(char *fn, char *type, uint32_t resource, uint32_t subresource,
		    uint32_t *offset)
{
	...
	*offset = 0;
	...
	rom_log("ROM: PE \"%s\" environment variable expansion failed (%d)\n", fn, result);
	return 0;
    }
#else
    char *new_fn = fn;
	...
	rom_log("PE signature short read (offset %d)\n", pe_offset);
    }

    /* Compare PE signature. */
    if (pe_signature != 0x00004550) {
	rom_log("PE signature mismatch (%08X at offset %d)\n", pe_signature, pe_offset);
	goto end;
    }
    ...
    if (fseek(fp, coff.SizeOfOptionalHeader, SEEK_CUR) < 0) {
	rom_log("COFF optional header seek failed\n");
	goto end;
    }
...
next_rde:
    ...
    /* Return ROM offset and size. This probably shouldn't assume the
       data RVA has the same base as the data entry RVA, but it works. */
    if (offset)
	*offset = section.PointerToRawData + (data.DataRVA - section.VirtualAddress);
    size = data.DataSize;
    rom_log("offset %08X size %d\n", *offset, size);
