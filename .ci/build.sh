...
then
	...
	# Build SDL2 for joystick support, FAudio and the SDL interface, with most components
	# disabled or dynamically loaded to remove the dependencies on PulseAudio and libdrm.
	if [ ! -d "SDL2-2.0.20" ]
	then
	...
	cmake -G Ninja -D SDL_SHARED=ON -D SDL_STATIC=OFF \
		...
		\
		-D "CMAKE_TOOLCHAIN_FILE=$cwd_root/toolchain.cmake" -D "CMAKE_INSTALL_PREFIX=$cwd_root/archive_tmp/usr" -S SDL2-2.0.20 -B sdlbuild || exit 99
...
EOF
	...
	project="$project" project_lower="$project_lower" project_version="$project_version" project_icon="$project_icon" arch_deb="$arch_deb" \
